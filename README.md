...
- **风格不统一**：剧本的写实基调、分镜的机位语言、场景图的材质光照各说各话，拼在一起不像一部戏
...
| 阶段 | 输入 | 输出 | 关键规则 |
|------|------|------|----------|
| 一 | 小说片段 | **横屏短剧剧本**（16:9，真人写实风） | 人物表+场景列表+统一设定表；逐场戏（场景/时长/镜头/画面内容/对白/环境音/运镜建议）；每集 6-12 场、每场 15-45 秒；心理描写 100% 外化为动作/表情/道具；环境音每场必标 |
...
- **写实铁律**：拒绝舞台剧式表演、拒绝旁白式心理叙述，所有内心活动转化为观众能看到的动作与道具
...
复制本项目目录（或下载 ZIP）。项目尚未发布到远程仓库，暂不支持 `git clone`；发布后可改用 git 方式获取。
...
**优先级约定**：你的显式指令（指定路径/文件名/篇幅）优先于 skill 默认写法；片段撑不满 6 场时按实际内容定场数，不会为凑数虚构剧情。
...
以下示例取自真实测试产出（`examples/slice-of-life/`），与 skill 实际行为一致。
...
- 开发阶段用上述三种类型片段做了端到端测试（每个测试由独立子代理加载 skill 完整执行），测试发现并修复的 14 条 SKILL.md 缺陷及修复记录见各测试报告与 [SKILL.md](skills/Novel-Switch-AI%20video%20prompts/SKILL.md) 中的"优先级规则"、"统一设定表"等章节。
