...
### 拿到场景提示词后的使用流程（v1.12.0，只需两次操作，空间唯一来源=锚点图）
...
**🎨 独立画风提示词（v1.12.0 新增）**：场景提示词文件末尾另附"六、画风提示词"小节——一段**纯画风**提示词（流派/渲染/光影/色调/质感/画质，零场景内容词）。复制它并拼接任意简短内容词（如 "a character portrait"），即可生成与场景**同画风**的角色图、道具图、衍生场景或海报，保证全项目视觉风格统一。
...
- 开发阶段做了两轮端到端测试（每个测试由独立子代理加载 skill 完整执行）：
  ...
  - **空间一致性（v1.4→v1.12）**：新增"空间基准表"（v1.4.0）；v1.5.0 锚点图特殊列出；v1.6.0 双锚定；v1.7.0 一整段提示词一次图生图；v1.8.0 排版由 AI 完成；v1.9.0 每格序号+名称图注；v1.10.0 空间唯一来源=锚点图；v1.11.0 图生图提示词只含机位+画面风格（零物品名词）；v1.11.1 图注"方位+功能词"命名；**v1.12.0 新增独立"画风提示词"小节（纯画风、零场景内容词，可复用于角色/道具/衍生图）**；已用雨夜档案馆验证（`场景AI绘图提示词_v1.12.0.md` 为最终版）；另用三种新片段做场景图专项测试（便利店/老宅/天台），测试产物已清理；已知局限——依赖工具参考图能力，不支持参考图的工具无法保证空间一致
