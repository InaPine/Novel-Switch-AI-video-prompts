...
# 雨夜档案馆 · 场景AI绘图提示词（v1.12.0）

> 依据阶段一《雨夜档案馆》剧本（6 场，水墨画风）生成。本文件为**阶段三场景提示词**；v1.12.0 关键改动：**新增「六、画风提示词」独立小节**——一段纯画风英文提示词（零场景内容词，含流派/渲染/光影/色调/质感/画质六要素），复制后拼接任意内容词即可复用为同画风的角色/道具/衍生图。第一、二部分沿用 v1.11.1 规则：锚点图为空间唯一来源、AI 一次排版、方位+功能词图注、全流程零物品名词。
...
- **图注命名（沿自 v1.11.1）**：每格图注为「方位+功能词」（NORTH ENTRY / CENTER AISLE / SOUTH END / CENTER FLOOR / SOUTH ROW / MID FLOOR），宏观标 MACRO；**全文（含图注）零物品名词**——不出现 DOOR / DOORWAY / WINDOW / SHELF 等物品词。
...
> v1.12.0 新增。本段是**纯画风提示词**：只描述「水墨画风」本身，不含任何场景内容词（无地点、物品、道具、人物、建筑结构），只回答"这张图是什么画风"。

**用户用途**：复制下面这段英文提示词，自行拼接任意简短内容词（如 `a character portrait`、`a teacup`），即可生成与场景**同画风**的其他图片（角色图、道具图、衍生场景、海报等），保证全项目视觉风格统一。本段可独立成图，不依赖场景提示词中的任何词。

```
Watercolor / ink-wash painting style, soft flowing ink textures, blurred softened outlines, dim cold-blue rainy-night ambient light with faint cool-green accents, low-saturation muted palette: ink-brown, cold grey-blue, paper cream, generous negative space, deep ink shadows, 8k hyper-detailed, unified color grading.
```
...
| 要素 | 内容 |
|------|------|
| ① 画风流派 | Watercolor / ink-wash painting style |
| ② 渲染方式 | soft flowing ink textures, blurred softened outlines |
| ③ 光影氛围 | dim cold-blue rainy-night ambient light with faint cool-green accents |
| ④ 色调 | low-saturation muted palette: ink-brown, cold grey-blue, paper cream |
| ⑤ 质感 | generous negative space, deep ink shadows |
| ⑥ 画质后缀 | 8k hyper-detailed, unified color grading |

> 本段**零场景内容词**：不出现 archive / library / room / shelf / door / window / footprint / gazetteer / corridor 等任何地点、物品、结构名词。
